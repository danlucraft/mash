...
class Mash < Hash
  ...
  def deep_update(other_hash)
    ...
    other_hash = other_hash.mash_stringify_keys
...
class Hash
  ...
  def mash_stringify_keys
    dup.mash_stringify_keys!
  end
  ...
  def mash_stringify_keys!
    keys.each{|k| 
      v = delete(k)
      self[k.to_s] = v
      v.mash_stringify_keys! if v.is_a?(Hash)
      v.each{|p| p.mash_stringify_keys! if p.is_a?(Hash)} if v.is_a?(Array)
    }
    self
  end
end